      ...
      * -the mandatory fields ADRESS-1, CITY, ADDR-STATE and ZIP-CODE
      *  are not empty;
      *   -ADDRESS TYPE
      *   -     must be one of the listed 88 level fields
      *   -ADDR-STATE must be in the STATE-ZIP table;
      *   -The address zip code must between
      *         two columns of zip codes;
      *   -Each possible ADDRESS-TYPE value may appear only once;
      *   -At least the LS-SUPP-ADDRESS-ONE must be populated.  Others
      *    can be blank, as long as blanks are at the end.
      *****************************************************************
       PROCEDURE DIVISION
           USING
                LS-SUPP-ADDRESS-ONE,
       ...
       100-INITIALIZATION.
      * Perform initialization pertaining to the entire SUPP-ADDRESSES
       ...
       110-INIT-STATE-ZIP-TABLE.
           IF NOT STATE-ZIP-LOADED *> Static; should only happen once
      ...
      * Initialize variables local to each address within the record
